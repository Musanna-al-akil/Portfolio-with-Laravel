							...
							<div class="table-responsive">
								<table class="table table-striped">
									...
									<tbody>
										...
										<td class=""><a href="">{!! Str::words($post->institute_name,7)!!}</a></td>
										
										<td>{{ \Carbon\Carbon::parse($post->created_at)->diffForHumans()}}</td>
                                        
                                       
                                       <td>
                                                <form style="display:none;" method="POST" id="ab-{{$post->id}}"action="{{route('EducationDelete',$post->id)}}">@csrf</form> 
                                        <a href="{{route('EditEducation',$post->id)}}" class="btn btn-warning"><i class="icon icon-pencil"></i></a>
					...
					<div class="modal-dialog" role="document">
					  <div class="modal-content">
						...
						<div class="modal-body">
						  ...
						  <p> {!! Str::words($post->institute_name,7)!!}</p>
						</div>
						<div class="modal-footer">
						  <button type="button" class="btn btn-secondary" data-dismiss="modal">No,Keep It</button>
						  <form  method="POST" id="ab-{{$post->id}}"action="{{route('EducationDelete',$post->id)}}">@csrf  
